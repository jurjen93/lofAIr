...
class DiffTrainer(Utils):
    ...
    def _setup_exec_env(self, virtual_device, ngpus_per_node, settings):
        ...
        net = GaussianDiffusion(net, steps=settings['steps'], sample_steps=settings['sample_steps']) # From diffusion.py
        ...
        settings['upsample'] = nn.Upsample(scale_factor=4.0, mode='bicubic').to(settings['device']) # Scale factor is set here again! 
    ...
    def _setup_train_env(self):
        ...
        # generate sample x_T
        generator = torch.Generator().manual_seed(2147483647) # to generate same sample x_T , why seeded?
