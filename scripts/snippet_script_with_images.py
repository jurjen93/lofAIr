...
def take_snippet(source_idx, r, ra, dec, cat_id, source_id, all_im_03_arcs, im_6_arcs_data, im_6_arcs_header, wcs_6_arcs):
    ...
    # Finding peak 
    im_6_max = np.max(cutout_6_image)
    im_03_max = np.max(cutout_03_image)
    diff_max = im_6_max-im_03_max
    
    cut_off_ps = 5e-4
    
    if np.isnan(diff_max) == False:
...
def main():
    ...
    for i in range(len(ra)): # Need to automatically find length again!
        take_snippet(i, 128, ra, dec, cat_id, source_id, all_im_03_arcs, im_6_arcs_data, im_6_arcs_header, wcs_6_arcs)
