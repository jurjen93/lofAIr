...
def cat_import(filename):
    ...
    fac = catelog[1].data['Facet'] # number for facet
    
    return source_id, cat_id, ra, dec, fac # Probably could also make this more efficient

def take_snippet(source_idx, r, ra, dec, fac, cat_id, source_id, im_03_arcs_data, im_03_arcs_header, wcs_03_arcs, im_6_arcs_data, im_6_arcs_header, wcs_6_arcs):
    """
    ...
    if diff_max > cut_off_ps:
        path_6_output = '/net/vdesk/data2/WoestE/output_Bootes/6resolution_output/Non_point_source/'
        path_03_output = '/net/vdesk/data2/WoestE/output_Bootes/03resolution_output/Non_point_source/'
    else:
        path_6_output = '/net/vdesk/data2/WoestE/output_Bootes/6resolution_output/Point_source/'
        path_03_output = '/net/vdesk/data2/WoestE/output_Bootes/03resolution_output/Point_source/'
      
    # Saving files
    hdu_6_arc = fits.PrimaryHDU(header=cutout_6_header, data=cutout_6_image)
    hdu_6_arc.writeto(str(path_6_output)+str(cat_id[source_idx])+"_Rad"+str(r)+"_6arcs.fits", overwrite=True)

    hdu_03_arc = fits.PrimaryHDU(header=cutout_03_header, data=cutout_03_image)
    hdu_03_arc.writeto(str(path_03_output)+str(cat_id[source_idx])+"_Rad"+str(r)+"_03arcs.fits", overwrite=True)
    ...
    # plt.colorbar()
    path_im = '/net/vdesk/data2/WoestE/output_Bootes/pngs/'
    plt.savefig(path_im+str(cat_id[source_idx])+"_Rad"+str(r)+".png")
    plt.clf()
...
def main():
    ...
    source_id, cat_id, ra, dec, fac = cat_import(args.cat)
    for i in range(len(ra)): # Need to automatically find length again!
        take_snippet(i, 64, ra, dec, fac, cat_id, source_id, im_03_arcs_data, im_03_arcs_header, wcs_03_arcs, im_6_arcs_data, im_6_arcs_header, wcs_6_arcs)
